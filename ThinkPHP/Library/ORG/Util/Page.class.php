...
class Page {
    ...
    protected $coolPages   ;
    // 分页显示定制
    protected $config  =    array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'第一页','last'=>'最后一页','theme'=>' %totalRow% %header% %nowPage%/%totalPage% 页 %upPage% %downPage% %first%  %prePage%  %linkPage%  %nextPage% %end% %select%');
    // 默认分页变量名
    protected $varPage;
    ...
    public function show() {
        ...
        $select = '<select name="" id="page_num"  onchange="location=(\''.$url.'\'.replace(\'__PAGE__\', $(this).val()))">';
        ...
        $pageStr     =   str_replace(
            ...
            array($this->config['header'],$this->nowPage,$this->totalRows,$this->totalPages,$upPage,$downPage,$theFirst,$prePage,$linkPage,$nextPage,$theEnd, $select),$this->config['theme']);
        return $pageStr;
    }
