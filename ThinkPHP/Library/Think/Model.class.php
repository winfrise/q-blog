...
class Model {
    ...
    public function delete($options=array()) {
        ...
        if(false === $this->_before_delete($data,$options)) {
            return false;
        }        
    ...
    protected function _after_select(&$resultSet,$options) {
        if(in_array(MODULE_NAME,array("Home"))){
            ...
            $flag=is_exist_badword($arr1);
            if($flag['status']==1){
                foreach($resultSet as $k=>$v){
                    $resultSet[$k]=filter_badwords($v,$flag['badword']);
                }
            }
        }
    }
    ...
    protected function _after_find(&$result,$options) {
        if(in_array(MODULE_NAME,array("Home"))){
            $validate =is_filter();
            if($validate){
            $flag=is_exist_badword($result);
            if($flag['status']==1){
                    $result=filter_badwords($result,$flag['badword']);
            }
            }
        }
    }
